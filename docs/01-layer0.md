...
층 0 확정 직후 왕복 절차 5단계(등급제 평가)를 소급 적용한 결과다. 이 단계는 층 0을 마친 뒤 작업자의 요청으로 절차에 추가되었다.

**(1) 지금 고쳐야 하는 것** — 없음. 층 0의 도출 자체에 이후 층의 전제를 틀어지게 할 구조적 오류는 없다. 다만 주의 하나: 선택 항목(자사 코어 시스템)은 전략으로는 옳지만, 고객사 하나가 동작하기 전에 공통 추상화부터 만들면 실체 없는 추상화가 된다. 구현 순서에는 넣지 않고, 층 2에서 "고객사마다 달라지는 부분과 공통인 부분의 경계"를 정할 때 참조 자료로만 쓴다.

**(2) 다음 층에서 스스로 드러날 것** — 두 곳.

- 결핍 목록의 **방향이 한쪽뿐이다.** 네 결핍 모두 "엔진이 고객사 시스템에 대해/대해서 무엇이 없는가"다. 이 도식의 엔진은 어떤 조건이 갖춰져도 스스로 일을 시작할 수 없고, 일을 끝낼 수도 없다. 무엇이 빠졌는지는 층 1의 질문이 그대로 겨냥하고 있으므로 여기서는 말하지 않는다.
- 결핍 3은 **정책이 조회 가능한 형태로 존재한다고 가정하고 있다.** 이 가정이 어디까지 성립하는지는 전환 2에서 데이터로 확인하게 된다.

**(3) 계획보다 나은 것** — 두 가지.

- **상자 대신 영역.** 계획 문서는 "구성 요소는 상자로만"이라고 썼지만, 층이 내려가며 내부가 채워지는 진행 방식에는 열린 영역이 더 맞다. 이후 층도 영역 방식을 유지한다.
- **목적을 먼저 정의하고 결핍을 물었다.** 계획 문서의 절차에는 없던 단계다. 결핍은 목적에 대해서만 정의되므로 이 순서가 방법론적으로 옳고, "법적으로 적절한 인터페이스"처럼 층 0에서 나오기 어려운 항목(권한과 책임의 문제)이 이 순서 덕에 나왔다.

## 6. 계획(00-approach.md)과 실제의 비교
